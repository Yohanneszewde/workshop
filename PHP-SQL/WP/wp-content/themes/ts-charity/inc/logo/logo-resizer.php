 ...
 */
function ts_charity_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size','ts-charity' ),
...
add_action( 'customize_register', 'ts_charity_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function ts_charity_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = ts_charity_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = ts_charity_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'ts_charity_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function ts_charity_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
 ...
 */
function ts_charity_customize_controls_js() {
	wp_enqueue_script( 'ts-charity-customizer-controls', get_template_directory_uri() . 'inc/logo/js/customize-controls.js', array( 'jquery', 'customize-preview' ), '201709071000', true );
}
add_action( 'customize_controls_enqueue_scripts', 'ts_charity_customize_controls_js' );

/**
 * Adds CSS to the Customizer controls.
 */
function ts_charity_customize_css() {
	wp_add_inline_style( 'customize-controls', '#customize-control-logo_size input[type=range] { width: 100%; }' );
}
add_action( 'customize_controls_enqueue_scripts', 'ts_charity_customize_css' );

/**
 * Testing function to remove logo_size theme mod
 */
function ts_charity_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'logo_size', '' );
	}
}
add_action( 'wp_loaded', 'ts_charity_remove_theme_mod' );